...
SELECT aircraft_code, COUNT(*) AS total_flights, ROUND(AVG(TIMESTAMPDIFF(MINUTE, scheduled_departure, scheduled_arrival))) AS avg_flight_duration_minutes
FROM flights GROUP BY aircraft_code;
...
SELECT tf.flight_id,f.flight_no, SUM(tf.amount) AS total_revenue FROM ticket_flights tf 
INNER JOIN flights f ON tf.flight_id = f.flight_id GROUP BY tf.flight_id, f.flight_no;
...
SELECT flight_id, ROUND(AVG(boarding_no)) AS avg_boarding_no, COUNT(*) AS total_boarding_passes
...
select f.flight_id,f.flight_no,f.aircraft_code,s.total_seats, count(bp.ticket_no) as occupied_seats,
...
inner join (select aircraft_code, count(seat_no) as total_seats from seats group by aircraft_code) s on f.aircraft_code = s.aircraft_code
group by f.flight_id,f.flight_no,f.aircraft_code,s.total_seats order by f.flight_id,f.aircraft_code;
...
SELECT tf.flight_id,f.flight_no, SUM(tf.amount) AS total_revenue FROM ticket_flights tf INNER JOIN flights f ON tf.flight_id = f.flight_id 
GROUP BY tf.flight_id, f.flight_no ORDER BY total_revenue DESC LIMIT 3;
...
SELECT a.model, ROUND(AVG(TIMESTAMPDIFF(MINUTE, f.scheduled_departure, f.scheduled_arrival)), 2) AS avg_duration_minutes
FROM flights f JOIN aircrafts a ON f.aircraft_code = a.aircraft_code GROUP BY a.model;
...
SELECT departure_airport, COUNT(*) AS departure_flights FROM flights GROUP BY departure_airport;
...
SELECT arrival_airport, COUNT(*) AS arrival_flights FROM flights GROUP BY arrival_airport;
...
SELECT departure_airport,arrival_airport, COUNT(*) AS flights_count FROM flights GROUP BY departure_airport, arrival_airport;
...
SELECT flight_id, COUNT(*) AS passengers_boarded FROM boarding_passes GROUP BY flight_id;
...
SELECT flight_id, ROUND(AVG(boarding_no)) AS avg_boarding_no
FROM boarding_passes GROUP BY flight_id;
...
SELECT t.passenger_id,COUNT(t.ticket_no) AS total_tickets, SUM(tf.amount) AS total_spent FROM tickets t 
INNER JOIN ticket_flights tf ON t.ticket_no = tf.ticket_no GROUP BY t.passenger_id;
...
SELECT fare_conditions, COUNT(*) AS tickets_sold, SUM(amount) AS total_revenue
FROM ticket_flights GROUP BY fare_conditions;
...
SELECT f.departure_airport,f.arrival_airport, SUM(tf.amount) AS total_route_revenue FROM flights f
JOIN ticket_flights tf ON f.flight_id = tf.flight_id GROUP BY f.departure_airport, f.arrival_airport;
...
SELECT HOUR(book_date) AS booking_hour, COUNT(*) AS bookings_count FROM bookings 
GROUP BY HOUR(book_date) ORDER BY bookings_count DESC LIMIT 5;
